...
class ProductUuidList(BaseModel):
    ...
    values: Optional[Dict[str, List[Dict[str, StrictStr]]]] = Field(None, description="Product attributes values, see <a href='/concepts/products.html#focus-on-the-product-values'>Product values</a> section for more details")
    associations: Optional[ProductUuidListAllOfAssociations] = None
    quantified_associations: Optional[ProductUuidListAllOfQuantifiedAssociations] = None
    ...
    metadata: Optional[ProductListAllOf1Metadata] = None
    ...
    completenesses: Optional[List[ProductListAllOf1Completenesses]] = Field(None, description="Product completenesses for each channel/locale combination (only available since the 7.0 version, and when the \"with_completenesses\" query parameter is set to \"true\")")
    __properties = ["_links", "uuid", "enabled", "family", "categories", "groups", "parent", "values", "associations", "quantified_associations", "created", "updated", "metadata", "quality_scores", "completenesses"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProductUuidList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ProductUuidList:
        """Create an instance of ProductUuidList from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return ProductUuidList.parse_obj(obj)

        _obj = ProductUuidList.parse_obj({
            ...
            "associations": ProductUuidListAllOfAssociations.from_dict(obj.get("associations")) if obj.get("associations") is not None else None,
            "quantified_associations": ProductUuidListAllOfQuantifiedAssociations.from_dict(obj.get("quantified_associations")) if obj.get("quantified_associations") is not None else None,
            "created": obj.get("created"),
            "updated": obj.get("updated"),
            "metadata": ProductListAllOf1Metadata.from_dict(obj.get("metadata")) if obj.get("metadata") is not None else None,
            "quality_scores": obj.get("quality_scores"),
            "completenesses": [ProductListAllOf1Completenesses.from_dict(_item) for _item in obj.get("completenesses")] if obj.get("completenesses") is not None else None
        })
        return _obj
